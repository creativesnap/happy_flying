(function ($) {
    ...
    // 
    $('.banner-one').css('height' , $(window).height() - 100)

    // navbar
    $('.main-nav__navigation-box a , .banner-one__btn , .service-one__top-content').on('click' , function() {
       $('html , body').animate({
           scrollTop : $('#' + $(this).data('scroll')).offset().top - $('.custom-navbar').innerHeight()
       })
    });

    (function animate(){
        $('.img-animate').animate({top:"-15px"},1000).animate({top:"0"},1000,function(){
            animate();
        });
    })();

    (function animate(){
        $('.paeashot-img').animate({top:"-150px"},1000).animate({top:"-125px"},1000,function(){
            animate();
        });
    })();
